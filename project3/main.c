...
void checkPlayer(u_int switches){
  ...
  unsigned char S3 = (switches & 8) ? 0 : 1;//Third switch
  ...
  else if(S3 && num != 0 && movePlayer != 1){ //Moves right
    x = 5;
  }
  else{ //Don't move
    x = 0;
  ...
  if(S2 && num == 0){ //Cover up starting screen
    drawString8x12(60,20, "I", COLOR_ORANGE, COLOR_BLACK);
    drawString8x12(45,50, "LOVE", COLOR_ORANGE, COLOR_BLACK);
    drawString8x12(30, 120, "CMP ARCH", COLOR_ORANGE, COLOR_BLACK);
...
int main(void)
{
  ...
  for(;;) {
    ...
    P1OUT |= GREEN_LED;       /**< Green led o
				 or_sr(0x10);        /**< CPU OFF */
  }
  P1OUT |= GREEN_LED;       /**< Green led on when CPU on */
  redrawScreen = 0;
  checkPlayer(reader);       //Constantly check which switch is down to move
